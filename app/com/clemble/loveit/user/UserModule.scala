...
class UserModule(env: PlayEnvironment, conf: Configuration) extends ScalaModule {
  ...
  @Named("user")
  @Singleton
  def userMongoCollection(collectionFactory: JSONCollectionFactory): JSONCollection = {
    collectionFactory.create("user")
  }

  @Provides
  @Named("invitation")
  @Singleton
  def inviteMongoCollection(collectionFactory: JSONCollectionFactory): JSONCollection = {
    collectionFactory.create("invitation")
  }
