...
object PaymentTransactionGenerator extends Generator[PaymentTransaction] {
  ...
  override def generate(): PaymentTransaction = {
    if (Random.nextBoolean()) {
      ...
      PaymentTransaction.credit(IDGenerator.generate(), UserGenerator.generate().id, Random.nextLong(), Money(Random.nextLong(), LoveItCurrency.getInstance("USD")), BankDetailsGenerator.generate())
    }
  }
